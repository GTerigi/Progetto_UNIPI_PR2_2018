...
public class MyDriveList<E> implements SecureDataContainer<E> {
	...
	private ArrayList<UserAccount> Drive;
	
			//COSTRUTTORE
	
	public MyDriveList() {
		Drive=new ArrayList<UserAccount>();
	}
	...
	public void createUser(String Id, String passw) {
		...
		if(Drive.isEmpty()) {
			...
			try {
				if(!containsUser(Id)) Drive.add(new UserAccount<E>(Id, passw));				
	...
	public void removeUser(String Id, String passw) {
		try {
			...
			for(UserAccount<E> element : Drive) {
				if(MatchUser(element, Id, passw)){
					Drive.remove(element);
	...
	public boolean put(String Owner, String passw, E data) {
		try {
			...
			if(Drive.isEmpty()) throw new Exception("Drive vuoto!");
			for(UserAccount<E> element : Drive) {
				if(MatchUser(element, Owner, passw)){
	...
	public E remove(String Owner, String passw, E data) {
		try {
			...
			for(UserAccount<E> element : Drive) {
				if(MatchUser(element, Owner, passw)){					
					E tmp=element.GetData(data);
					if(element.RemoveData(data)) {
	...
	public void copy(String Owner, String passw, E data) {
		try {
			...
			for(UserAccount<E> element : Drive) {
				if(MatchUser(element, Owner, passw)) {
					E elem=element.GetData(data);
					System.out.println("Il dato "+ elem +" è stato copiato\nDimenzione attuale:" + getSize(Owner, passw));
					element.SetData(elem);
					return;
				}
			}
			System.out.println("Nessun utente di nome " + Owner + " trovato");
	...
	public void share(String Owner, String passw, String Other, E data) {
		try {
			if(Drive==null) throw new NullPointerException("Errore, Drive non inizializzato");
			if(Drive.isEmpty()) throw new Exception("Drive vuoto!");
			UserAccount<E> tmp=null;
            E datatocopy = null;
			for(UserAccount<E> element : Drive) {
				if(Other.equals(element.GetUser())) {
					tmp=element;
				}
				if(MatchUser(element, Owner, passw)) {
					datatocopy=element.GetData(data);
				}
			}
			if(tmp!=null && datatocopy!=null) {
				System.out.println("Il dato "+ datatocopy +" è stato condiviso con "+ Other + "\nDimenzione attuale:" + getSize(Owner, passw));
				tmp.SetData(datatocopy);
	            return;
			}
	...
	public int getSize(String Owner, String passw) {
		try {
			if(Drive==null) throw new NullPointerException("Errore, Drive non inizializzato");
			if(Drive.isEmpty()) throw new Exception("Drive vuoto!");
			else for(UserAccount<E> element : Drive) {
				if(element.equalsOwner(Owner)) {
					if(element.MatchPassword(passw)) return element.GetSize();
				}
			}
		}
		catch(NullPointerException e) {
	 ...
	 * THROWS: if Drive==null throws NullPointerException
	 * 		   if Drive is empty throws Exception
			...
			for(UserAccount<E> element : Drive)  {
				if(MatchUser(element, Owner, passw)) {
					if(element.GetSize()>0) {
						return element.GetIterator();
					}
					else throw new NullPointerException("Errore, drive dell'account vuoto");
				}
			}
		}
		catch(NullPointerException e) {
	...
	private boolean containsUser(String Id) {
		for(UserAccount<E> element: Drive) {
			if(Id.equals(element.GetUser())) return true;
		}
		return false;
	}
